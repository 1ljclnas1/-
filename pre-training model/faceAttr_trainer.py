...
class Classifier_Trainer(object):
    ...
    def evaluate(self):  # 在测试集上进行评估性能
        ...
        matrix = {}  # 创建一个空矩阵存储混淆矩阵
        for attr_idx, attr in enumerate(self.selected_attrs):
            if attr_idx != 5:
                conf_matrix = torch.zeros(2, 2)
                matrix[attr] = conf_matrix
        with torch.no_grad(): # 在该模块下，所有计算得出的tensor的requires_grad都自动设置为False。即，反向传播时就不会自动求导了
    ...
    def fit(self, model_path=None):
        ...
        #     ans = labels[0]
        #     conf_matrix = torch.zeros(2, 2)
        #     self.confusion_matrix(preds=ans, labels=ans, conf_matrix=conf_matrix)
        # 如果有参数文件
        ...
        for attr_idx, attr in enumerate(self.selected_attrs):
            print(best_label_dict[attr])
            C = confusion_matrix(best_label_dict[attr], best_predict_dict[attr])
            print(C)
